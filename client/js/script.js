...
function loadMore(){
  ...
  if(i==27){
    ...
    y.innerText = "Zatvori dodatne kartice";
...
let j = 0;
function searchList() {
  // Prikazivanje ili skrivanje liste pretrage
...
function searchFilter() {
  ...
  for (var j = 0; j < li.length; j++) {
      var listItem = li[j].getElementsByClassName("list-item");
      var itemInfo = listItem[0].getElementsByClassName("item-info");
      var h2 = itemInfo[0].getElementsByTagName("h2");
      ...
      if (a[0]) {
          ...
          if (textValue.toUpperCase().indexOf(filter) > -1) {
              li[j].style.display = ""; // Prikazuje stavku
          } else {
              li[j].style.display = "none"; // Sakriva stavku
          }
      }
  }
}
